...
@section('content')
    ...
    <x-layouts.mobile-app class="bg-[#F8FAFB] min-h-screen pb-32" x-data="{
        ...
        harga: {{ old('base_harga', $layanan->base_harga ?? 0) }},
        diskon: {{ old('diskon_persentase', $layanan->diskon_persentase ?? 0) }},
        homeCare: {{ old('homecare_harga', $layanan->homecare_harga ?? 0) }},
        ...
        <div class="px-6 pt-8 space-y-8">
            ...
            <div class="space-y-3 px-1">
                ...
                <p class="text-sm font-medium text-slate-500 leading-relaxed">
                    Perbarui atau hapus katalog layanan cabang Anda dengan presisi.
                </p>
            </div>
            ...
            {{-- 3. FORM GROUPS --}}
            <form action="{{ route('admin-cabang.layanan.update', $layanan->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="deskripsi" x-model="deskripsi" rows="6"
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium text-slate-650 leading-relaxed focus:ring-4 focus:ring-teal-500/10 focus:bg-white transition-all outline-none shadow-inner resize-none"></textarea>
                    </div>
                </div>
                ...
                <div class="space-y-3 pt-4">
                    ...
                    <button type="button" @click="showDeleteModal = true"
                        class="w-full py-4 bg-rose-50 text-rose-500 rounded-[1.5rem] text-sm font-black uppercase tracking-[0.2em] active:scale-95 transition-all border border-rose-100">
                        Hapus Layanan
                    </button>
            ...
            <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[999] flex items-center justify-center p-6"
                ...
                <div class="relative bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl space-y-6"
                    ...
                    <div class="text-center space-y-3">
                        <div class="w-16 h-16 mx-auto bg-rose-50 rounded-2xl flex items-center justify-center text-rose-500">
                        ...
                        <p class="text-sm text-slate-500">Layanan <strong x-text="namaLayanan"></strong> akan dihapus secara permanen dari katalog cabang Anda. Tindakan ini tidak dapat dibatalkan.</p>
                    </div>

                    <div class="space-y-3">
                        <form action="{{ route('admin-cabang.layanan.destroy', $layanan->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="w-full py-4 bg-rose-500 text-white rounded-2xl text-sm font-black uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-rose-500/20">
                                Ya, Hapus
                            </button>
                        </form>
