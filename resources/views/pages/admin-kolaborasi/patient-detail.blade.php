    ...
    <div class="px-6 pt-6 space-y-6">
        ...
        <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-4">
            ...
            <div class="space-y-1">
                ...
                <div class="flex items-center justify-center gap-2">
                    <span class="px-2.5 py-0.5 bg-teal-50 text-teal-600 text-[9px] font-black uppercase tracking-widest rounded-md border border-teal-100">
                        {{ $patient->membership_tier ?: 'Basic' }}
                    </span>
                    <span class="px-2.5 py-0.5 bg-slate-50 text-slate-500 text-[9px] font-black uppercase tracking-widest rounded-md border border-slate-100">
        ...
        <div class="bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm grid grid-cols-3 gap-1">
            <button @click="activeTab = 'personal'"
                :class="activeTab === 'personal' ? 'bg-teal-800 text-white shadow-md' : 'text-slate-400 hover:text-slate-700'"
            ...
            <button @click="activeTab = 'bookings'"
                :class="activeTab === 'bookings' ? 'bg-teal-800 text-white shadow-md' : 'text-slate-400 hover:text-slate-700'"
                class="py-3 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all duration-300">
                Booking
            </button>
            <button @click="activeTab = 'medical'"
                :class="activeTab === 'medical' ? 'bg-teal-800 text-white shadow-md' : 'text-slate-400 hover:text-slate-700'"
                class="py-3 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all duration-300">
                Medis
            </button>
        </div>
        ...
        <div x-show="activeTab === 'personal'" x-transition class="space-y-6">
            <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-4">
                <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em] border-b border-slate-50 pb-2">Informasi Kontak</h3>
                ...
                <div class="grid grid-cols-2 gap-y-4 gap-x-2">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs font-bold text-slate-700">{{ $patient->jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                    </div>
                    <div class="space-y-1">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs font-bold text-slate-700">{{ $patient->tanggal_lahir ? $patient->tanggal_lahir->translatedFormat('d F Y') : '-' }}</p>
                    </div>
                    <div class="space-y-1">
                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Umur</span>
                        <p class="text-xs font-bold text-slate-700">{{ $patient->age ? $patient->age . ' Tahun' : '-' }}</p>
                    </div>
            ...
            <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-4">
                <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em] border-b border-slate-50 pb-2">Kesehatan & Membership</h3>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs font-bold text-slate-700">{{ $patient->tinggi_badan ? $patient->tinggi_badan . ' cm' : '-' }}</p>
                    </div>
                    <div class="space-y-1">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs font-bold text-slate-700">{{ $patient->berat_badan ? $patient->berat_badan . ' kg' : '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'bookings'" x-transition class="space-y-4">
            <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em] px-1">Riwayat Booking ({{ $bookings->count() }})</h3>
            
            @forelse($bookings as $bk)
                <div class="bg-white rounded-[1.8rem] p-5 border border-slate-100 shadow-sm space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="px-2 py-0.5 bg-slate-50 text-slate-500 text-[8px] font-black uppercase tracking-wider rounded border border-slate-200">{{ $bk['id'] }}</span>
                        ...
                        @php
                            ...
                            $statusColor = $colorMap[strtolower($bk['status'])] ?? 'bg-slate-50 text-slate-500 border-slate-100';
                        @endphp
                        
                        <span class="px-2.5 py-0.5 text-[8px] font-black uppercase tracking-widest rounded border {{ $statusColor }}">
                            {{ $bk['status'] }}
                        </span>
                    </div>
        ...
        <div x-show="activeTab === 'medical'" x-transition class="space-y-4">
            <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em] px-1">Riwayat Rekam Medis ({{ $medicalRecords->count() }})</h3>
                ...
                <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-50 pb-2">
                        <div class="space-y-0.5">
                            ...
                            <p class="text-[9px] font-medium text-slate-400">{{ $med['tanggal'] }} • {{ $med['jam'] }}</p>
                        </div>
                        <div class="text-right">
                            <span class="text-[8px] font-bold text-slate-400 uppercase block tracking-wider">Terapis</span>
                    ...
                    <div class="grid grid-cols-2 gap-3 text-xs leading-relaxed">
                        <div class="space-y-0.5">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Tensi</span>
                            <p class="font-bold text-slate-700">{{ $med['tensi'] }}</p>
                        </div>
                        <div class="space-y-0.5">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Skala Nyeri</span>
                            <p class="font-bold text-slate-700">{{ $med['skala_nyeri'] }}/10</p>
                        </div>
                        <div class="col-span-2 space-y-0.5">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Keluhan Pasien</span>
                            <p class="font-medium text-slate-600 bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">{{ $med['keluhan'] }}</p>
                        </div>
                        <div class="col-span-2 space-y-0.5">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Catatan Terapis</span>
                            <p class="font-medium text-slate-600 bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">{{ $med['catatan_terapis'] }}</p>
                        </div>
                        <div class="col-span-2 space-y-0.5">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Saran & Rekomendasi</span>
                            <p class="font-medium text-slate-600 bg-teal-50/20 p-2.5 rounded-xl border border-teal-100/20">{{ $med['saran'] }}</p>
                        </div>
                    </div>
                </div>
            @empty
