        ...
        <div
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('admin-global.cabang.menu', $kolaborasi->id) }}"
                    class="p-2 -ml-2 text-slate-400 hover:text-teal-600 transition">
        ...
        <div class="px-6 pt-8 pb-32 space-y-8">
            ...
            <div class="space-y-3 px-1">
                ...
                <p class="text-sm font-medium text-slate-500 leading-relaxed">
                    Informasi jam buka dan tutup untuk membantu merencanakan kunjungan atau sesi konsultasi berikutnya
                    secara lebih mudah.
                </p>
            </div>

            {{-- 3. FORM --}}
            <form action="{{ route('admin-global.operasional-jadwal.update', $kolaborasi->id) }}" method="POST"
                class="space-y-6">
                @csrf
                        ...
                        <div class="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm transition-all duration-300 overflow-hidden"
                            ...
                            <div class="px-6 py-5 flex justify-between items-center">
                                <div class="flex items-center gap-3">
                                    ...
                                    <h3 class="text-base font-black text-slate-700 uppercase tracking-widest"
                                        x-text="day.name"></h3>
                                </div>
                            ...
                            <div x-show="day.active" x-collapse x-cloak class="px-6 pb-6 space-y-6">
                                <div class="flex items-center gap-3">
                                    <div class="flex-1 grid grid-cols-2 gap-3">
                                        ...
                                        <div class="space-y-2">
                                            <label
                                                class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Buka</label>
                                            <div class="relative flex items-center">
                                                <input type="time" x-model="day.open"
                                                    class="w-full px-4 py-3 bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-teal-500/20">
                                            </div>
                                        </div>
                                        {{-- Tutup --}}
                                        <div class="space-y-2">
                                            <label
                                                class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Tutup</label>
                                            <div class="relative flex items-center">
                                                <input type="time" x-model="day.close"
                                                    class="w-full px-4 py-3 bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-teal-500/20">
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div x-show="day.hasBreak" x-transition class="space-y-3">
                                        <div class="relative flex items-center justify-center">
                                            ...
                                            <span
                                                class="relative px-3 bg-white text-[9px] font-black text-slate-300 uppercase tracking-[0.3em]">Jam
                                                Istirahat</span>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            <div class="flex-1 grid grid-cols-2 gap-3">
                                                <input type="time" x-model="day.breakStart"
                                                    class="w-full px-4 py-3 bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700">
                                                <input type="time" x-model="day.breakEnd"
                                                    class="w-full px-4 py-3 bg-slate-100 border-none rounded-xl text-sm font-bold text-slate-700">
                                            </div>
                                            <button type="button" @click="day.hasBreak = false"
                ...
                <div class="space-y-3 pt-6">
                    <button type="submit"
                        class="w-full py-5 bg-teal-800 text-white rounded-xl text-sm font-black uppercase tracking-[0.2em] shadow-xl shadow-teal-900/20 active:scale-95 transition-all">
                        Simpan Jam Operasional
                    </button>
                    <a href="{{ route('admin-global.cabang.menu', $kolaborasi->id) }}"
                        class="block w-full py-4 bg-slate-200/50 text-slate-500 rounded-xl text-sm font-black uppercase tracking-[0.2em] active:scale-95 transition-all text-center">
                        Batal
                    </a>
                </div>
            </form>
