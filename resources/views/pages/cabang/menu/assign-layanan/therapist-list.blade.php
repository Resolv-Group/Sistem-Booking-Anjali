...
<x-layouts.mobile-app class="bg-[#F8FAFB] min-h-screen" x-data="{ 
    search: '',
    therapists: [
        { id: 1, name: 'Dr. Elena', role: 'Spesialis Akupunktur', rating: '4.9', reviews: 128, count: 2, status: 'Tersedia hari ini' },
        { id: 2, name: 'Dr. Aris Muh.', role: 'Spesialis Akupunktur', rating: '4.8', reviews: 95, count: 4, status: 'Tersedia hari ini' },
        { id: 3, name: 'Dr. Rachel', role: 'Spesialis Akupunktur', rating: '5.0', reviews: 210, count: 3, status: 'Tersedia hari ini' }
    ],
    get filteredTherapists() {
        return this.therapists.filter(t => t.name.toLowerCase().includes(this.search.toLowerCase()));
    }
}">
    ...
    <x-ui.topbar title="Rumah Terapi Anjali">
            <x-slot:left>
                <a href="{{ route('admin-global.cabang.menu') }}" class="p-1 -ml-1 text-slate-400 hover:text-teal-600 transition">
    ...
    <div class="px-6 pt-8 pb-32 space-y-8">
        ...
        <div class="relative group">
            <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-teal-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <template x-for="t in filteredTherapists" :key="t.id">
                <div class="bg-white rounded-[1.5rem] p-6 border border-slate-100 shadow-sm space-y-5">
                    ...
                    <div class="flex justify-end">
                        <span class="flex items-center gap-1.5 text-[9px] font-black text-teal-600 uppercase tracking-widest">
                    ...
                    <div class="flex items-center gap-5">
                        <div class="w-16 h-16 rounded-2xl bg-slate-100 border-2 border-white shadow-md overflow-hidden shrink-0">
                            <img :src="`https://i.pravatar.cc/150?u=${t.id}`" class="w-full h-full object-cover">
                        </div>
                        <div class="space-y-1">
                            <h4 class="text-lg font-black text-slate-800 leading-none" x-text="t.name"></h4>
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-widest" x-text="t.role"></p>
                            <div class="flex items-center gap-1 text-amber-400 pt-0.5">
                                ...
                                <span class="text-[10px] font-bold text-slate-300 uppercase ml-0.5" x-text="`(${t.reviews} Ulasan)`"></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-3">
                        <div class="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-teal-600 shadow-sm">
                        ...
                        <span class="text-xs font-black text-slate-600 uppercase tracking-widest" x-text="`Mengelola ${t.count} Layanan`"></span>
                    </div>

                    <a x-on:click="window.location.href = '{{ route('admin-global.assign-layanan') }}'" class="block w-full py-4 bg-[#005F5C] text-white text-center rounded-xl text-sm font-black uppercase tracking-[0.2em] shadow-lg shadow-teal-900/20 active:scale-95 transition-all">
                        Kelola Layanan
                    </a>
                </div>
            </template>
