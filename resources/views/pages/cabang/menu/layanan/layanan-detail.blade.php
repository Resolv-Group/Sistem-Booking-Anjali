...
<x-layouts.mobile-app class="bg-[#F8FAFB] min-h-screen" x-data="{ 
    ...
    <div class="px-6 py-5 flex justify-between items-center bg-white/90 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-100">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin-global.layanan', $kolaborasi->id) }}" class="p-1 -ml-1 text-slate-400 hover:text-teal-600 transition">
    ...
    <div class="px-6 pt-8 pb-32 space-y-8">
        ...
        {{-- 3. FORM GROUPS --}}
        <form action="{{ route('admin-global.layanan.update', [$kolaborasi->id, $layanan->id]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-3 pt-4">
                ...
                <a href="{{ route('admin-global.layanan', $kolaborasi->id) }}" class="block w-full py-4 bg-slate-200/50 text-slate-500 rounded-[1.5rem] text-sm font-black uppercase tracking-[0.2em] active:scale-95 transition-all text-center">
                    Batal
                </a>
            </div>
        </form>
        ...
        <div x-show="showDeleteModal" x-cloak
            ...
            <div class="relative bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl space-y-6"
                ...
                <div class="text-center space-y-3">
                    ...
                    <p class="text-sm text-slate-500">Layanan <strong x-text="namaLayanan"></strong> akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
                </div>

                <div class="space-y-3">
                    <form action="{{ route('admin-global.layanan.destroy', [$kolaborasi->id, $layanan->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
