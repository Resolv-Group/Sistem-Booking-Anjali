    ...
    <x-layouts.mobile-app class="bg-[#F8FAFB] min-h-screen pb-32" x-data="{
        photoPreview: '{{ $therapist->foto
            ...
            : 'https://ui-avatars.com/api/?name=' .
                urlencode($therapist->nama_karyawan) .
                '&background=0d766e&color=fff&size=128' }}',
        gender: '{{ old('jenis_kelamin', $therapist->jenis_kelamin ?? 'L') }}',
        openDropdown: false,
        showPw: false,
        showPwConf: false
    }">
            ...
            <form action="{{ route('therapist.profile.update') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-5">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nama_karyawan"
                            value="{{ old('nama_karyawan', $therapist->nama_karyawan) }}"
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nomor NIK
                            (KTP)</label>
                        <input type="text" name="nik" value="{{ old('nik', $therapist->nik) }}" maxlength="16"
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold text-slate-700 focus:ring-4 focus:ring-teal-500/10 transition-all outline-none">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="tanggal_lahir"
                                value="{{ old('tanggal_lahir', $therapist->tanggal_lahir ? $therapist->tanggal_lahir->format('Y-m-d') : '') }}"
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-5">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email
                            Aktif</label>
                        <input type="email" name="email" value="{{ old('email', $therapist->email) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="tel" name="no_telp" value="{{ old('no_telp', $therapist->no_telp) }}"
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold text-slate-700 outline-none">
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-2">
                    ...
                    <textarea name="alamat" rows="3"
                        ...
                        placeholder="Contoh: Jl. Mawar No. 123, Surabaya...">{{ old('alamat', $therapist->alamat) }}</textarea>
                </div>
