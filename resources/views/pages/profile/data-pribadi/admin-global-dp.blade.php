...
@section('content')
    <x-layouts.mobile-app class="bg-[#F8FAFB] min-h-screen pb-32" x-data="{
        ...
        <div class="px-6 pt-8 space-y-8">
            ...
            <form action="{{ route('admin-global.profile.update') }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col items-center space-y-4">
                    <div class="relative">
                        <div
                            ...
                            <img :src="photoPreview" class="w-full h-full object-cover"
                                alt="{{ $admin_global->nama_karyawan }}">
                        </div>
                    ...
                    <div class="text-center space-y-1">
                        {{-- <p class="text-xs font-black text-teal-600 uppercase tracking-widest">
                            {{ $admin_global->pasien_public_id }}</p> --}}
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-5">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nama_karyawan"
                            value="{{ old('nama_karyawan', $admin_global->nama_karyawan) }}"
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nomor NIK
                            (KTP)</label>
                        <input type="text" name="nik" value="{{ old('nik', $admin_global->nik) }}" maxlength="16"
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold text-slate-700 focus:ring-4 focus:ring-teal-500/10 transition-all outline-none">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="tanggal_lahir"
                                value="{{ old('tanggal_lahir', $admin_global->tanggal_lahir ? $admin_global->tanggal_lahir->format('Y-m-d') : '') }}"
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-5">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email
                            Aktif</label>
                        <input type="email" name="email" value="{{ old('email', $admin_global->email) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="tel" name="no_telp" value="{{ old('no_telp', $admin_global->no_telp) }}"
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold text-slate-700 outline-none">
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm space-y-2">
                    ...
                    <textarea name="alamat" rows="3"
                        ...
                        placeholder="Contoh: Jl. Mawar No. 123, Surabaya...">{{ old('alamat', $admin_global->alamat) }}</textarea>
                </div>
