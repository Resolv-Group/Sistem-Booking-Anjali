@props([
    ...
    'required' => false, // Tambahkan prop baru untuk required
])
...
<div class="space-y-2" x-data="{
    ...
    init() {
        ...
        // 1. Jika ada 'old' value, gunakan itu.
        // 2. Jika tidak ada 'old' value tapi ada opsi tersedia, pilih opsi pertama secara otomatis.
        if (this.selected) {
            this.label = options[this.selected] || '{{ $placeholder }}';
        } else if (keys.length > 0) {
            this.selected = keys[0];
            this.label = options[keys[0]];
        }
    }
    ...
    @if ($label)
        <label class="mb-1.5 block text-sm font-semibold text-gray-700 ml-1">
            {{ $label }}
            {{-- Tambahkan tanda * jika required berharga true --}}
            @if ($required)
    ...
    <div class="relative">
        ...
        <button type="button" @click="open = !open"
            ...
            :class="open ?
                '{{ $name && $errors->has($name) ? '!border-red-500 ring-4 ring-red-500/20' : 'border-primary ring-4 ring-primary/10' }}' :
                ''">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200"
            ...
            @foreach ($options as $value => $text)
                <div @click="selected = '{{ $value }}'; label = '{{ $text }}'; open = false"
                    class="cursor-pointer rounded-xl px-4 py-3 text-base transition-colors hover:bg-primary/10 hover:text-primary font-medium"
                    :class="selected === '{{ $value }}' ? 'bg-primary text-white hover:bg-primary hover:text-white' :
                        'text-gray-700'">
                    {{ $text }}
                </div>
            @endforeach
